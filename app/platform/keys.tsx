...
function KeyEditor({ existing, available, context, close, save }: { existing?: KeyRecord; available: Model[]; context: Context; close: () => void; save: (key: KeyRecord) => string | null }) {
  ...
  const noModelsSelected = Object.keys(policies).length === 0;
  ...
  return <Dialog title={existing ? '编辑 API Key' : '创建 API Key'} close={close} wide className={context === 'personal' ? 'p-personal-key-dialog' : undefined}><form onSubmit={submit}><div className="p-dialog-body"><label className="p-field"><span><i>*</i> API Key 名称</span><div className="p-count-input"><input data-autofocus name="key-name" autoComplete="off" aria-label="API Key 名称" maxLength={50} value={name} placeholder="请输入" onChange={e => setName(e.target.value)} /><span>{name.length} /50</span></div></label>
    {context === 'enterprise' ? <fieldset className="p-model-field" aria-invalid={noModelsSelected} aria-describedby={noModelsSelected ? 'key-model-error' : undefined}><legend><i>*</i> 授权模型与并发上限</legend><p className="p-muted">至少选择一个企业已授权模型。每个 Key 的上限不预留并发资源。</p><div className="p-policy-head"><span>模型</span><span>Key 并发上限</span></div>{available.map(m => { const selected = m.id in policies; const value = policies[m.id]; const bad = selected && (!Number.isInteger(value) || value < 1 || value > m.limit); return <div className={`p-policy-row ${bad ? 'p-policy-error' : ''}`} key={m.id}><label><Checkbox checked={selected} onCheckedChange={checked => { if (checked) setPolicies({ ...policies, [m.id]: m.limit }); else { const next = { ...policies }; delete next[m.id]; setPolicies(next); } }} /><span>{m.id}<small>企业上限 {m.limit} 路</small></span></label><div><input type="number" aria-label={`${m.id} 并发上限`} aria-invalid={bad} aria-describedby={bad ? `error-${m.id}` : undefined} min={1} max={m.limit} step={1} disabled={!selected} value={selected && !Number.isNaN(value) ? value : ''} onChange={e => setPolicies({ ...policies, [m.id]: e.target.value === '' ? NaN : Number(e.target.value) })} /><span>路</span>{bad ? <span className="p-sr-only" id={`error-${m.id}`}>请输入 1 至 {m.limit} 的整数</span> : null}</div></div>; })}{noModelsSelected ? <p id="key-model-error" className="p-error" role="alert">请至少选择一个授权模型。</p> : null}</fieldset> : <fieldset className="p-model-field p-personal-model-field" aria-invalid={noModelsSelected} aria-describedby={noModelsSelected ? 'key-model-error' : undefined}><legend><i>*</i> 授权模型</legend><p className="p-muted">选择此 Key 可以调用的模型，至少选择一个。</p><div className="p-personal-model-grid">{available.map(m => { const selected = m.id in policies; return <label className={`p-personal-model-card ${selected ? 'is-selected' : ''}`} key={m.id}><Checkbox checked={selected} onCheckedChange={checked => setPolicies(current => { const next = { ...current }; if (checked) next[m.id] = m.limit; else delete next[m.id]; return next; })} /><span>{m.id}</span></label>; })}</div>{noModelsSelected ? <p id="key-model-error" className="p-error" role="alert">请至少选择一个授权模型。</p> : null}</fieldset>}
