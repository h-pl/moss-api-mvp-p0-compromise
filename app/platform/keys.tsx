...
export default function Keys({ keys, context, save, viewUsage }: { keys: KeyRecord[]; context: Context; save: (key: KeyRecord) => string | null; viewUsage: (id: string) => void }) {
  ...
  return <main id="main-content" className="p-main p-keys">
    ...
    <Notice>完整密钥只在创建时展示一次。如怀疑泄露，请立即删除该 Key 并创建新的 Key。</Notice>
    {context === 'enterprise' ? <RekaResourceDisclosure title={<span className="p-resource-title"><Icon name="building" />企业授权模型 <span className="p-muted">{available.length} 个</span><Help text="同一模型的所有 API Key 共享企业并发额度。各 Key 的并发上限之和可以超过企业上限，但实际同时请求总数不能超过企业上限。" /></span>}><div className="p-resource-grid">{available.map(m => <div key={m.id}><span className="p-model-ellipsis" title={m.id}>{m.id}</span><strong>{m.limit}<small> 路共享并发</small></strong></div>)}</div></RekaResourceDisclosure> : null}
    ...
    <div className={`p-key-table p-table-scroll ${context === 'enterprise' ? 'p-enterprise-keys' : 'p-personal-keys'}`}><table><colgroup><col className="p-col-name" /><col className="p-col-key" />{context === 'enterprise' ? <col className="p-col-models" /> : null}<col className="p-col-note" /><col className="p-col-created" /><col className="p-col-status" /><col className="p-col-actions" /></colgroup><thead><tr><th>名称</th><th>密钥</th>{context === 'enterprise' ? <th>授权模型 / 并发上限</th> : null}<th>备注</th><th>创建时间</th><th>状态</th><th>操作</th></tr></thead><tbody>{filtered.slice((currentPage - 1) * pageSize, currentPage * pageSize).map(k => <tr key={k.id}><td><button className="p-name-button" onClick={() => setEditor(k)}>{k.name}</button></td><td><code>{k.masked}</code></td>{context === 'enterprise' ? <td><div className="p-model-lines">{Object.entries(k.policies).map(([id, limit]) => <div key={id}><span>{id}</span><b>{limit} <small>路</small></b></div>)}</div></td> : null}<td><span className="p-note-cell" title={k.note}>{k.note || '—'}</span></td><td className="p-nowrap">{timeLabel(k.createdAt).replaceAll('-', '.')}</td><td><span className={`p-status ${k.enabled ? 'p-status-on' : ''}`}>{k.enabled ? '已启用' : '已停用'}</span></td><td><div className="p-actions"><ToggleSwitch checked={k.enabled} label={`${k.enabled ? '停用' : '启用'} ${k.name}`} onCheckedChange={() => setAction({ key: k, type: 'toggle' })} /><button className="p-icon-button" aria-label={`编辑 ${k.name}`} title="编辑" onClick={() => setEditor(k)}><Icon name="edit" /></button><button className="p-icon-button" aria-label={`查看 ${k.name} 的用量`} title="查看用量与并发" onClick={() => viewUsage(k.id)}><Icon name="chart" /></button><button className="p-icon-button" aria-label={`删除 ${k.name}`} title="删除" onClick={() => setAction({ key: k, type: 'delete' })}><Icon name="trash" /></button></div></td></tr>)}</tbody></table>{!filtered.length ? <Empty>{current.length ? '没有匹配的 API Key，请调整搜索或筛选条件。' : '尚未创建 API Key，点击右上角创建。'}</Empty> : null}</div>
