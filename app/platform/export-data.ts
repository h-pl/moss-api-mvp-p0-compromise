...
export function usageExport(kind: 'keys' | 'requests', rows: RequestRecord[], keys: KeyRecord[], account: string, filters: Filters) {
  ...
  if (kind === 'requests') {
    ...
    const data = rows.map(r => {
      ...
      return [...base, timeLabel(r.start), r.id, r.model, r.keyId, key?.name ?? '', key?.masked ?? '', key?.note ?? '', '/api/v2/tasks', r.status, r.units, models.find(model => model.id === r.model)?.unit ?? '', (r.cents / 100).toFixed(2), r.rate ? ratePoints(r.rate) : '', r.rate?.basisLabel ?? '', r.rate ? discountLabel(r.rate.discountBps) : '', estimateYuan(r.cents).toFixed(4)];
    });
    return { headers, data };
  }
