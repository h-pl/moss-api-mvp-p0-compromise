...
export default function Usage({ context, keys, requests, initialKey, balance, getCredits }: { context: Context; keys: KeyRecord[]; requests: RequestRecord[]; initialKey: string; balance: number; getCredits: () => void }) {
  ...
  return <main id="main-content" className="p-main p-usage"><div className="p-usage-title"><span>账单与用量</span><h1>用量与积分</h1></div>
    ...
    {context === 'enterprise' ? <section className="p-panel p-concurrency" aria-label="近 24 小时并发峰值"><div className="p-section-heading"><div><h2>近 24h 并发峰值 <Help text={`${keyId === 'all' ? `${context === 'enterprise' ? '企业' : '个人'}全部 Key` : selectedKey?.name}：展示各模型近 24 小时的并发峰值及当前${keyId === 'all' ? (context === 'enterprise' ? '企业' : '账户') : 'Key'}并发上限。`} /></h2></div><small>更新于 {timeLabel(now).slice(5, 16)}</small></div>
